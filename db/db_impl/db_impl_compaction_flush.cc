  ...
  *made_progress = false;
  ...
  if (!c) {
    ...
    *made_progress = true;
    ...
    // Should handle error?
	// byhs::start
	//printf("[DEBUG] compaction job run2\n");
	// byhs::end
    compaction_job.Run().PermitUncheckedError();
	...
	// byhs:: this is a temporary variable 
    /* const std::vector<CompactionInputFiles>* testcf = c.get()->inputs();

    std::cout << "size1:" << (*testcf).size() << " size2:" << (*testcf)[0].files.size() << " input level:" << (*testcf)[0].level << std::endl;

	  for(int i=0; i<(int)(*testcf).size(); i++){
	    for(int j=0; j<(int)(*testcf)[i].files.size(); j++){
        //std::cout << "name(" << i << ", " << j << ") " << TableFileName(c.get()->immutable_options()->db_paths, (*testcf)[i].files[j]->fd.GetNumber(), (*testcf)[i].files[j]->fd.GetPathId()) << std::endl;
        std::string remove_sst = "rm -rf " + TableFileName(c.get()->immutable_options()->db_paths, (*testcf)[i].files[j]->fd.GetNumber(), (*testcf)[i].files[j]->fd.GetPathId());
        [[maybe_unused]]int rtv = system(remove_sst.c_str());
      }
    }
    *///Annoted for testing (by byhs) 
    status =
        compaction_job.Install(*c->mutable_cf_options(), &compaction_released);
